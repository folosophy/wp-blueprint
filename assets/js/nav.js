...
function fsyMobileNav() {
  ...
  var $ham = $('.nav-main .toggle, .menu-main__toggle'),
...
class NavMain {
  ...
  watch() {
    ...
    $(window).scroll(function() {
      ...
      else {self.$body.removeClass('is-scrolling');}

      if (st > 800) {

        if (st > self.ls) {
          self.$nav.addClass('is-hidden').removeClass('is-visible');
        } else {self.$nav.removeClass('is-hidden').addClass('is-visible');}
      ...
      if (st > 200) {self.$body.addClass('below-menu');}
      else {self.$body.removeClass('below-menu');}
